...
- Exactly one consumer *decides* anything on it: `derive_progress`'s IDK-009 §9.2 precedence check (`domain.py:378`). The other three read-sites only carry the value between layers — `service.py:389` copies it from the rubric dimension onto the result at assessment time, and `repository.py:318` / `repository.py:795` write and read the column. Nothing else branches on it.
