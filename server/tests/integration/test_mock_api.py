...
def test_mock_complete_rejects_blank_and_is_idempotent_after_enqueue_failure(
    ...
    evidence_before = client.get(
        f"/api/v1/goals/{run['goal_id']}/evidence"
    ).json()
    blank = client.post(
...
def test_mock_next_turn_cancel_preserves_transcript_and_db_rejects_hint(
    ...
    with uow_factory() as uow:
        ...
        submitted = submit_mock_answer(
            uow, owner.id, run["id"], exact, "mock-next-job"
        )
        before = tuple((turn.id, turn.kind.value, turn.body) for turn in submitted.turns)
    ...
    with engine.begin() as connection:
        owner_id = connection.execute(text("SELECT owner_id FROM interview_runs WHERE id=:id"), {"id": run["id"]}).scalar_one()
        try:
...
def test_mock_adaptive_turn_and_unchanged_completion_guard(client, uow_factory):
    ...
    assert client.get(f"/api/v1/interview-runs/{run['id']}").json()["turns"] == after["turns"]
