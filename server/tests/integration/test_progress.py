...
def _goal(client: TestClient, uow_factory: UnitOfWorkFactory) -> str:
    with uow_factory() as uow:
        ...
        uow.canonical.create_topic_identity(
            TopicIdentity("progress-topic", "progress-topic", timestamp, None)
        )
...
def test_progress_uses_injected_authoritative_clock_and_exposes_fixture_status(
    client: TestClient, uow_factory: UnitOfWorkFactory
):
    client.app.state.clock = FixedClock()
    goal_id = _goal(client, uow_factory)
    response = client.get(
        f"/api/v1/goals/{goal_id}/progress", params={"now": "1999-01-01T00:00:00Z"}
    )
...
def test_database_invalidates_memo_and_rejects_correction_branches(
    ...
    with engine.connect() as connection:
        assert (
            connection.execute(
                text("SELECT count(*) FROM goal_progress_memos WHERE goal_id=:g"),
                {"g": goal_id},
            ).scalar_one()
            == 1
        )
    with engine.begin() as connection:
        connection.execute(
            text(
                "INSERT INTO learner_corrections(id,owner_id,goal_id,topic_stable_id,correction_type,body_hash,created_at) SELECT 'c1',:o,:g,t.stable_id,'correction','hash-c1',:at FROM topics t JOIN goal_workspaces w ON w.graph_version_id=t.graph_version_id WHERE w.id=:g LIMIT 1"
            ),
            {"o": owner_id, "g": goal_id, "at": "2026-08-01T00:00:00.000000Z"},
        )
        connection.execute(
            text(
                "INSERT INTO learner_correction_bodies(correction_id,owner_id,goal_id,value,reason) VALUES ('c1',:o,:g,'partial',NULL)"
            ),
            {"o": owner_id, "g": goal_id},
        )
    with engine.connect() as connection:
        assert (
            connection.execute(
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                "INSERT INTO learner_corrections(id,owner_id,goal_id,topic_stable_id,correction_type,body_hash,created_at,supersedes_correction_id) SELECT 'c2',:o,:g,topic_stable_id,'gap','hash-c2',:at,'c1' FROM learner_corrections WHERE id='c1'"
            ),
            {"o": owner_id, "g": goal_id, "at": "2026-08-02T00:00:00.000000Z"},
        )
        connection.execute(
            text(
                "INSERT INTO learner_correction_bodies(correction_id,owner_id,goal_id,value,reason) VALUES ('c2',:o,:g,'new',NULL)"
            ),
            {"o": owner_id, "g": goal_id},
        )
    with (
        engine.begin() as connection,
        pytest.raises(IntegrityError, match="active same-scope leaf"),
    ):
        connection.execute(
            text(
                "INSERT INTO learner_corrections(id,owner_id,goal_id,topic_stable_id,correction_type,body_hash,created_at,supersedes_correction_id) SELECT 'c3',:o,:g,topic_stable_id,'gap','hash-c3',:at,'c1' FROM learner_corrections WHERE id='c1'"
            ),
            {"o": owner_id, "g": goal_id, "at": "2026-08-03T00:00:00.000000Z"},
        )
    states = client.get(f"/api/v1/goals/{goal_id}/learning-state-explanations").json()[
        "learning_states"
    ]
    ...
    with engine.begin() as connection:
        assert (
            connection.execute(
                text("SELECT count(*) FROM learner_corrections WHERE goal_id=:g"),
                {"g": goal_id},
            ).scalar_one()
            == 2
        )
        connection.execute(
            text("UPDATE goal_progress_memos SET input_hash='stale' WHERE goal_id=:g"),
            {"g": goal_id},
        )
...
def test_memo_hits_within_fixture_day_and_invalidates_at_day_boundary(
    ...
    with engine.connect() as connection:
        first_stored_at = connection.execute(
            text("SELECT computed_at FROM goal_progress_memos WHERE goal_id=:g"),
            {"g": goal_id},
        ).scalar_one()
...
def test_matching_hash_corrupt_memo_is_recomputed(
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                "UPDATE goal_progress_memo_bodies SET explanation_json='{}' WHERE goal_id=:g"
            ),
            {"g": goal_id},
        )
    recovered = client.get(f"/api/v1/goals/{goal_id}/progress").json()
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                "UPDATE goal_progress_memo_bodies SET coverage='partial' WHERE goal_id=:g"
            ),
            {"g": goal_id},
        )
    ...
    with engine.begin() as connection:
        raw = connection.execute(
            text(
                "SELECT explanation_json FROM goal_progress_memo_bodies WHERE goal_id=:g"
            ),
            {"g": goal_id},
        ).scalar_one()
        corrupted = raw.replace(
            "Fixture-only derivation", "Corrupt but schema-valid definition", 1
        )
        connection.execute(
            text(
                "UPDATE goal_progress_memo_bodies SET explanation_json=:value WHERE goal_id=:g"
            ),
    ...
    with engine.connect() as connection:
        assert (
            connection.execute(
                text("SELECT coverage FROM goal_progress_memo_bodies WHERE goal_id=:g"),
                {"g": goal_id},
            ).scalar_one()
