...
@router.post("/interview-runs", response_model=PracticeRunResponse, status_code=201)
...
@router.get("/interview-runs/{run_id}", response_model=PracticeRunResponse)
def read_interview_run(
    run_id: str,
    owner_id: Annotated[str, Depends(get_owner_id)],
    uow: Annotated[InterviewUnitOfWork, Depends(get_unit_of_work)],
):
    return _run_response(get_practice_run(uow, owner_id, run_id))
...
@router.post(
    "/interview-runs/{run_id}/cancel-evaluation", response_model=PracticeRunResponse
)
def post_interview_cancel(
    ...
    run = cancel_evaluation(uow, owner_id, run_id, clock=clock)
    uow.commit()
    return _run_response(run)
