...
function ImportsPage({ state, setState }: { state: OperationsState; setState: React.Dispatch<React.SetStateAction<OperationsState>> }) {
  ...
  const onboardingMaterial = learning.onboarding.sourceMaterial
  const canCopyOnboardingMaterial = Boolean(onboardingMaterial.trim()) && onboardingMaterial !== state.importSource
  ...
  return <>
    ...
    {canCopyOnboardingMaterial && <section className="so-handoff"><div><span className="so-kicker">Saved during onboarding</span><strong>Source material is available to copy into import review.</strong><p>This explicit copy replaces the text area contents. It does not parse, map, verify, or accept the material.</p></div><Button tone="secondary" onClick={() => setState((s) => ({ ...s, importSource: onboardingMaterial }))}>Copy into review</Button></section>}
