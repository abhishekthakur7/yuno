...
function Practice({ navigate, selection }: PageProps) {
  ...
  return <Classroom navigate={navigate}><article className="sb-practice" aria-live="polite"><PageIntro eyebrow={`Guided practice · ${run?.state ?? 'ready'}`} title="Reason through the failure boundary"><span>Hints appear only when requested. Feedback appears only after submission and server evaluation.</span></PageIntro><JobConnectionStatus ids={[run?.active_job_id]} /><section className="sb-question"><span>Scenario</span><h2>{run?.question ?? question.question}</h2></section>
